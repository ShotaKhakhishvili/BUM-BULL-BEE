 ...
 * trigger color the bot stops, backs off the line, then spins away from
...
/*
 * Line reflex: stop, reverse off the line for APP_LINE_BACKUP_MS, then rotate in
 * place for APP_LINE_ROTATE_MS in rotate_dir, then stop. Normal movement resumes
 * on the next loop tick. This blocks for ~1 s, during which nothing else runs -
 * acceptable for a boundary escape (and intentional: the edge always wins over
 * seeking), but see the note if you need sensing live.
 */
